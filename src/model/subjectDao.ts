...
export function insertSubject(subject: Subject): Promise<any> {
  const insert = 'INSERT INTO Subject (name, userID) VALUES (?,?)';
  const selectQuery = 'SELECT last_insert_rowid() as id';

  return new Promise((resolve, reject) => {
    db.run(insert, [subject.getName(), subject.getUserID()], (err: { message: any; }) => {
      if (err) {
        reject(err.message);
      } else {
 ...
 */
export function updateSubject(subjectID: number, name: string, userID: number): Promise<String> {
  const update = 'UPDATE Subject SET name=?, userID=? WHERE subjectID=?';
  
  return new Promise((resolve, reject) => {
    db.run(update, [name, subjectID, userID], (err: { message: any; }) => {
      if (err) {
        reject(err.message);
      } else {
        resolve('worked');
